...
#if defined(NDEBUG)
# error "Fakecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc91802259e1aed6425235c7560114afd3f9ee591c147924109d9bf6e1606f903");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Fakecoin: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Fakecoin: 10k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Fakecoin: 1 days
static const int64 nTargetSpacing = 1.5 * 60; // Fakecoin: 1.5 minutes
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8e48c539c9475ed663b460833b1e882a584dddf0f1a0246c59e9bf6348906a81");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Developed By Bisto Man";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ee8b18c7fadc02c47aa97dcecdba75804feb38f9458cf7eb2898a93a86aca19179b802984e54ab9796c821f15a83e7f2102ec592c84c61edac01795c766a1e70") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1522671427;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085289599;

        if (fTestNet)
        {
            block.nTime    = 1522671372;
            block.nNonce   = 387059170;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x95d87d4b902ad0f98b138be22672e50e37fc3496d07490baa141ffb5abdbc849"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc5, 0xb4, 0xd2 }; // Fakecoin: increase each by adding 2 to bitcoin's value.
...
void static FakecoinMiner(CWallet *pwallet)
{
    printf("FakecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("fakecoin-miner");
    ...
    try { loop {
        ...
        printf("Running FakecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FakecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FakecoinMiner, pwallet));
}
